...
import com.Liu.utils.SharedPreferencesHelper;
import com.Liu.utils.SharedPreferencesTag;
...
public class UpdateReceiver extends BroadcastReceiver {
	...
	public static final String UPDATE_ACTION = "com.liuzhao.NewsApplication";
	...
	public void onReceive(Context context, Intent intent) {
		...
		mSharedPreferencesHelper = mSharedPreferencesHelper
				.getInstance(NewApplication.getInstance());
		HashMap<String, Object> tempMap = NewApplication.getInstance()
				.getTempMap();
		UpdateInfoModel model = (UpdateInfoModel) tempMap
				.get(DeliverConsts.KEY_APP_UPDATE);
		try {
			UpdateInformation.localVersion = NewApplication
					.getInstance()
					.getPackageManager()
					.getPackageInfo(
							NewApplication.getInstance()
									.getPackageName(), 0).versionCode;
			UpdateInformation.versionName = NewApplication
					.getInstance()
					.getPackageManager()
					.getPackageInfo(
							NewApplication.getInstance()
		...
		UpdateInformation.appname = NewApplication.getInstance()
