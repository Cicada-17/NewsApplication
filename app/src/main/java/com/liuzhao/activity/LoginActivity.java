...
import com.liuzhao.utils.DESUtil;
...
public class LoginActivity extends BaseActivity implements View.OnClickListener,View.OnLongClickListener {
    ...
    private HashMap<String, Object> tempMap = NewApplication.getInstance().getTempMap();
...
//   startActivity(intent);
            ...
            case R.id.bt_pwd_eye:
                if(et_pass.getInputType() == (InputType.TYPE_CLASS_TEXT|InputType.TYPE_TEXT_VARIATION_PASSWORD)){
                    bt_pwd_eye.setBackgroundResource(R.drawable.tab_four);
    ...
    private void newuser(){
        ...
        map.put("phone",et_name.getText().toString().trim());
        map.put("password", DESUtil.encode("Cniao5_123456",et_pass.getText().toString().trim()));
        OkhttpManager.postAsync(RequestURL.SIGN_URL, map, new OkhttpManager.DataCallBack() {
            @Override
            public void requestFailure(Request request, IOException e) {
                Log.d("liuzhaosign","注册失败");
            }

            @Override
            public void requestSuccess(String result) {
                Log.d("liuzhaosign","注册成功"+result);
    ...
    private void login() {
        ...
        map.put("phone",et_name.getText().toString().trim());
        map.put("password", DESUtil.encode("Cniao5_123456",et_pass.getText().toString().trim()));
        OkhttpManager.postAsync(RequestURL.LOGIN_URL, map, new OkhttpManager.DataCallBack() {
            @Override
            public void requestFailure(Request request, IOException e) {
                Log.d("liuzhaologin","请求失败");
            }

            @Override
            public void requestSuccess(String result) {
                Log.d("liuzhaologin","返回信息"+result);
                //解析数据
                person = new Person();
                try {
                    ...
                    if (status.equals("1")){
                       ...
                       // person.setLogo_url(person_object.getString("logo_url"));
                        person.setNobi(person_object.getString("mobi"));
